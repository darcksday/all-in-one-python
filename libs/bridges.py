...
def bridge(privatekey, amount, RPC, chain_id, scan):
    try:
        ...
        if chain_id == 42170 or chain_id == 324:
            gasLimit = web3.eth.estimate_gas({
                'to': Web3.toChecksumAddress(address_wallet),
                'from': Web3.toChecksumAddress(address_wallet),
                'value': web3.toWei(0.0001, 'ether')}) + random.randint(50000, 100000)
        else:
            gasLimit = web3.eth.estimate_gas(contract_txn)

        if chain_id == 324:
            contract_txn['gas'] = int(gasLimit)
        else:
            contract_txn['gas'] = int(gasLimit + gasLimit * 0.1)
...
def orbiter_bridge(privatekey, from_chain, to_chain, AMOUNT_TO_BRIDGE, MIN_BALANCE):
    ...
    if AMOUNT_TO_BRIDGE >= 0.015:
        ...
        cprint('Error: Bridge amount less than 0.015', 'red')
